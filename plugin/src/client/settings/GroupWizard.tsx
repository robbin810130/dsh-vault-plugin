...
export function GroupWizard({ store, onClose }: GroupWizardProps) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (Array.from(password).length < 8) { setError('密码至少需要 8 个字符'); return }
  ...
  return (
    <form className="dsh-vault-settings-panel" onSubmit={submit}>
      ...
      <label className="dsh-vault-field" htmlFor="dsh-vault-group-password"><span>密码</span><input id="dsh-vault-group-password" type="password" minLength={8} value={password} onChange={event => setPassword(event.currentTarget.value)} /></label>
      ...
      <div className="dsh-vault-dialog-actions"><button type="button" className="dsh-vault-button" onClick={onClose}>取消</button><button type="submit" className="dsh-vault-button dsh-vault-button-primary" disabled={pending || name.trim().length === 0 || password.length < 8 || confirmation.length < 8}>创建密码组</button></div>
    </form>
  )
}
