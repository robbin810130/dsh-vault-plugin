...
- Task 7: in progress. Rebased on `9745aef`; the current tag `v0.2.8` points at commit `91b73a8`. GitHub run `36019739012` passed all plugin and installer tests, then caught an invalid manually appended patch hunk before build. Restored the original patch and verified the full patch against all five live upstream preimages; the official commit and complete tree remain pinned. The CSS Modules fix supplies a stable repository-relative filename to Lightning CSS. Focused verifier tests pass 5/5. Remaining: rerun the tag workflow, refresh the three UI bundle hashes from CI output, rerun until the checked build passes, publish/verify release assets, and leave the launcher idle.
